...
if page == pages[0]:
    ...
    with st.container(border=True):
        st.image("/workspaces/Streamlit/Questions.png")
...
énergétique, etc.""")
    ...
    with col1:
        with st.container(border=True):
            st.image("/workspaces/Streamlit/Capture d’écran 2023-12-09 à 23.16.23.png")
    with col2:
        with st.container(border=True):
            st.image("/workspaces/Streamlit/Capture d’écran 2023-12-09 à 23.16.35.png")
    st.divider()
...
actions ont été réalisées à l’identique pour tous les fichiers utilisés.""")
    with st.container(border=True):
        st.image("/workspaces/Streamlit/Capture d’écran 2023-12-09 à 23.19.42.png", caption = "Étapes du traitement des données")
...
if page == pages[2]:
    ...
    with tab1:
        ...
        st.divider()
        df1 = pd.read_csv("/workspaces/Streamlit/GLB.Ts+dSST.csv", skiprows = 1)
        ...
        df_zon = pd.read_csv("/workspaces/Streamlit/ZonAnn.Ts+dSST.csv", index_col = 0)
        ...
        col1, col2 = st.columns(2)
        with col1:
            df_NHGRAPH = pd.read_csv("/workspaces/Streamlit/df_NHGRAPH.csv")
        ...
        with col2:
            df_HSGRAPH = pd.read_csv("/workspaces/Streamlit/df_HSGRAPH.csv")
        ...
        st.divider()
        df_ZonGRAPH = pd.read_csv("/workspaces/Streamlit/df_ZonGRAPH.csv")
    ...
    with tab2:
        ...
        df_co2 = pd.read_csv("/workspaces/Streamlit/OWID.csv")
    ...
    with tab4:
        ...
        if option == "Températures - PIB":
            ...
            df_corr_temp_pib = pd.read_csv("/workspaces/Streamlit/corr_temp_pib")
        ...
        if option == "PIB - CO2":
            ...
            df_corr_co2_pib = pd.read_csv("/workspaces/Streamlit/corr_co2_PIB.csv")
        ...
        if option == "CO2 - Températures":
            ...
            df_corr_co2_temp = pd.read_csv("/workspaces/Streamlit/corr_CO2_TEMP.csv")
...
if page == pages[3]:
    ...
    # TEMPERATURES
    regressor = load("/workspaces/Streamlit/Regression_lineaire_temp.joblib")
    df_plotly = pd.read_csv("/workspaces/Streamlit/DF_PLOTLY.csv")
    df_ml = pd.read_csv("/workspaces/Streamlit/DF_ML_TEMP.csv")
    ...
    #CO2 
    regressor_co2 = load("/workspaces/Streamlit/Regression_lineaire_co2.joblib")
    df_plotly_co2 = pd.read_csv("/workspaces/Streamlit/DF_PLOTLY_CO2")
    df_ml_co2 = pd.read_csv("/workspaces/Streamlit/DF_ML_CO2")
    ...
    with tab1:
        ...
        with st.container(border=True):
            st.image("/workspaces/Streamlit/Resultats_1.png")
        ...
        st.image("/workspaces/Streamlit/variables importantes.png")
        st.divider()
        ...
        with st.container(border=True):
            st.image("/workspaces/Streamlit/Resultats_2.png")
        st.divider()
